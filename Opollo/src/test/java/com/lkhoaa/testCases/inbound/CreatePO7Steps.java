...
//        System.out.println("Enter SKU POMS7STEP");
        driver.findElement(By.xpath("//input[@placeholder='Enter SKU here']")).sendKeys("POMS7STEP");
...
//        System.out.println("Confirm create PO");
        ...
        /*Verify PO steps*/
        System.out.println("Verify that the PO has 7 steps");
        boolean isStep7Displayed = false;
        boolean isStep8Displayed = false;

        try {
            WebElement step7Element = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[@class='step-text' and text()='7']")));
            isStep7Displayed = step7Element.isDisplayed();
        } catch (TimeoutException e) {
            // Step 6 not found - leave boolean as false
        }
        ...
        try {
            ...
            // Step 7 not found - leave boolean as false
        }

        if (isStep7Displayed && isStep8Displayed) {
            System.out.println("FAIL: Both Steps 7 and 8 are visible");
            softAssert.fail("Unexpected dual step visibility");
        } else if (isStep7Displayed) {
            System.out.println("PASS: Step 7 displayed (Step 8 hidden as expected)");
            ...
            System.out.println("CRITICAL FAIL: Step 7 missing");
            softAssert.fail("Step 7 not found");
        }

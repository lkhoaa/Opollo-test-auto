    ...
    public static double sumOfLzdPaymentFee(String filePath) {
        ...
        try {
            ...
            for (Row row : sheet) {
                ...
                if (feeStr != null && feeStr.equals("Payment Fee")) {
                ...
                if (feeStr2 != null && feeStr2.contains("Reversal Payment")) {
                    // Get the amount from the specified column
                    Cell amountCell = row.getCell(amountColumnIndex);
                    double amountValue = amountCell.getNumericCellValue();
                    if (amountCell != null) {
                ...
                if (feeStr3 != null && feeStr3.contains("Payment fee - correction for undercharge")) {
                    // Get the amount from the specified column
                    Cell amountCell = row.getCell(amountColumnIndex);
                    double amountValue = amountCell.getNumericCellValue();
                    if (amountCell != null) {
            ...
            System.out.printf("Total sum of Payment Fee + Reversal + correction for undercharge on LZD: ");
